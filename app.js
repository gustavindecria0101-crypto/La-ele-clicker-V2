...
function renderLeaderboard(){
  ...
  top.forEach((entry, idx) => {
    ...
    avatarImg.src = (users[entry.name] && users[entry.name].avatar) ? users[entry.name].avatar : '/noob.png';
...
setTimeout(()=>{ 
  ...
  if(!currentUser){
    ...
    if(!guest){
      ...
      const users = loadUsers();
      users[guest] = users[guest] || { password: '', avatar: '/noob.png' };
      saveUsers(users);
    }
...
window.addEventListener('load', ()=> {
  ...
  const SKINS = [
    { id: 'default', name: 'Padrão', src: '/OIP (1).webp', boost: 0 }, // default no boost
